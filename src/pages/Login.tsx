            ...
            <form
                ...
                <div className={`bottom-wrapper w-full`}>
                    <button
                        ...
                        <span
                            className={`absolute -top-[12px] left-1/2 bg-white/60 h-[0.5px] w-[96%] -translate-y-1/2 -translate-x-1/2`}></span> {isSignUp ? 'Sign Up' : "Login"}
                    </button>
                    <p className={`text-white netflixFontVariable text-sm mt-3 `}>{isSignUp ? 'Already registered?' : 'New to Netflix?'}
                        <span onClick={() => setIsSignUp(prev => !prev)}
                              className={`text-[#db0000] hover:cursor-pointer`}>{isSignUp ? 'Sign Up' : 'Login In'}</span>
                    </p>
